            ...
            if time >= start and time <= end:
                if G_snapshot.has_edge(edge[0], edge[1]):
                    ...
                    G_snapshot.add_edge(
                        ...
                        edge[1],
                        weight=1,
                        books=G[edge[0]][edge[1]]["books"],
                        bt=G[edge[0]][edge[1]]["bt"],
...
def dynamic_community_detection(
    snapshots,
    snapshot_times,
    parameters,
    randomize,
    use_init_partition,
    use_init_partition_randomize,
):
        ...
        if randomize:
            ...
            for x in range(10):
                coms = algorithms.louvain(
                    snapshot, randomize=None
                )  # here any CDlib algorithm can be applied
            ...
            if last_partition is not None:
                coms = algorithms.louvain(
                    snapshot, partition=list_to_dict(last_partition.communities)
                )  # here any CDlib algorithm can be applied
                last_partition = coms
            else:
            ...
            if last_partition is not None:
                ...
                best_coms = algorithms.louvain(snapshot, randomize=None)
                best_mod = evaluation.newman_girvan_modularity(
                    snapshot, best_coms
                ).score
                for x in range(10):
                    coms = algorithms.louvain(
...
def execute_dcd(sgs, key):
    ...
    dcd_init_partition = dynamic_community_detection(
        sgs[0],
        sgs[1],
        key,
        randomize=False,
        use_init_partition=True,
        use_init_partition_randomize=False,
    )
            ...
            if len(matches) == 0:
                ...
                for match in matches:
                    if fronts[match].split()[0] == str(t):
                        print("SPLIT")
                        dynamic_coms[uid] = dynamic_coms[match][:-1].append(
                            str(t) + "_" + str(index)
                        )
                        fronts[uid] = str(t) + "_" + str(index)
                        uid += 1
...
def print_dyn_com(dynamic_coms, id, tc, snapshots, snapshot_times):
    ...
    for part in dynamic_com:
        ...
        dc_centrality_dict_sorted = sorted(
            dc_centrality_dict.items(), key=lambda item: item[1], reverse=True
        )

        for value in dc_centrality_dict_sorted:
            print(
                f"{value[1][1]} - Centrality: {round(value[1][0], 3)} - URI: {value[0]}"
            )
        print("-------------------------")
...
def community_detection_louvain(g):
    ...
    avg_embeddedness_louvain_lcc = evaluation.avg_embeddedness(
        g_lcc, louvain_communities_lcc
    )

    # Print some analysis on the resulting clustering
    print(f"Number of communities (louvain): {len(louvain_communities.communities)}")
    print(
        f"Number of communities (louvain_lcc): {len(louvain_communities_lcc.communities)}"
    )
    print()
...
def format_partition(g, coms):
    ...
    for cluster in coms.communities:
        ...
        dc_sorted_communities_list.append(
            (dc_centrality_dict_sorted, min_max_dates, bookset, bookcount)
        )
    if not is_cached:
...
def export(sorted_communities, metapath_name, attribute_cache):
    ...
    with open(file_to_write, mode="w") as communities:
        for index1, community in enumerate(sorted_communities):
            communities.write(
                f"Community {index1+1}, total members of the community: {len(community[0])}, Period: {community[1]}, Number of books: {community[3]}"
            )
            communities.write("\n")
    ...
    with open(file_to_write, mode="w") as communities:
        ...
        coms_writer = csv.writer(
            communities, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
    ...
    with open(file_to_write, mode="w") as communities:
        ...
        for index1, community in enumerate(sorted_communities):
            coms_writer.writerow(
                [
                    f"Community {index1+1} - Period: ({community[1][0]} - {community[1][1]} - Number of books: {community[3]}"
                ]
            )
            # coms_writer.writerow()
            ...
            for book in community[2]:
                coms_writer.writerow(
                    [attribute_cache[book.uri], book.uri, book.bt, book.et]
                )
