...
BPB = """SELECT DISTINCT ?w1 ?w2 ?bt ?et ?b
                    WHERE { 
                        ?b  <https://schema.org/publication>/<https://schema.org/publishedBy> ?w1.
                        ?b   <https://schema.org/publication>/<https://schema.org/publishedBy> ?w2.
                        ?b  <https://schema.org/publication> ?pe.
...
BCB = """SELECT DISTINCT ?w1 ?w2 ?bt ?et ?b
                    WHERE { 
                        ?b  <https://schema.org/contributor> ?w1.
                        ?b  <https://schema.org/contributor> ?w2.
                        ?b  <https://schema.org/publication> ?pe.
                        OPTIONAL {?pe <http://semanticweb.cs.vu.nl/2009/11/sem/hasEarliestBeginTimestamp> ?bt}.
                        OPTIONAL {?pe <http://semanticweb.cs.vu.nl/2009/11/sem/hasLatestEndTimestamp> ?et}.
                        FILTER(?w1 != ?w2).
...
def main():
    ...
    for key, item in metapaths.items():
        ...
        sgs = {"40_10": sg_40_10, "20_10": sg_20_10, "10_1": sg_10_1}
        theta = 0.3
